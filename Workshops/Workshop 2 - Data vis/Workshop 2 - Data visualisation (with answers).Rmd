...
```{r}
mboto <- boto[boto$sex == "Male",]
fboto <- boto[boto$sex == "Female",]
...
Remember those violin plots from earlier, they're basically just density plots.
...
# In truth, it's actually a pretty clever y-axis. 100% represents a doubling in the number of buses; Increasing the number of buses from 5 to 10 is a 100% increase - the number has doubled. Going from 10 to 5, however, is a -50% decrease - the number has halved.

# People get super agitated by y-axes, especially when the bottom is not set to zero. Honestly? I don't care. There are times when it's entirely appropriate to do that, other times when it's not. But I really dislike when people seem to treat it as the *rule* is that there is only ever a singularly correct way to set up a figure. There's not. Use your brain. Think what the best way is to visualise your data and do that.

# As a small side note, the Financial Times uses ggplot2 to produce their figures and have their own theme (ftplottools).
```
...
```{r}
# A clearly silly figure. The bars here are just the order. So Finland is the "happiest" country in the world, while New Zealand is the 10th happiest. At this point, you've made a glorified list.

# More subtly, I'm always super skeptical of these kinds of "happiness" ratings. I have no idea how happy I am; am I a 10 out of 10 today or a 9 out of 10? Have I ever truly had a 0 out of 10? Would me and you give the same score if we had the same "happiness"? Maybe your 10/10 is my 5/10. What if I'm just having an off day and I'm pissed off? How many people do you ask to figure out happiness?

# Point is that the data underlying this figure is most likely nonsense. At least, it doesn't pass *my* assumption of validity check.
```
...
```{r}
# So I don't know what point the creator of this figure was trying to make, but if we ignore that and just consider the way the information is visualised, then there are some issues we might want to correct.

# First the y-axis are 
```
