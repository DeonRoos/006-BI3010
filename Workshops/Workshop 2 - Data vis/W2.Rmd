...
#   geom_point(aes(x = drought, y = length)) +
#   facet_wrap(~sex)
write.table(boto, here("Workshops/Workshop 2 - Data vis/Data", "alligator_bite.txt"), row.names = FALSE)
```
...
# height and width are specified inside the aesethics - should be outside
```
...
```{r, include=answer_sheet, class.source="bg-success", eval = F}
# No - there's nothing that jumps out as being suspicious.
```
